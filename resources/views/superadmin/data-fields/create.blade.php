...
@section('title', 'Tambah Field - ' . $dataType->name)
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Tambah Field untuk {{ $dataType->name }}</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('superadmin.data-types.fields.store', $dataType) }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <div class="form-text">Nama label yang akan ditampilkan di form dan tabel.</div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <div class="form-text">Kosongkan untuk generate otomatis dari Label. Gunakan snake_case.</div>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required onchange="toggleOptionsField()">
                        ...
                        <option value="text" {{ old('type') === 'text' ? 'selected' : '' }}>Text (Input Teks)</option>
                        ...
                        <option value="date" {{ old('type') === 'date' ? 'selected' : '' }}>Date (Tanggal)</option>
                        ...
                        <option value="dropdown" {{ old('type') === 'dropdown' ? 'selected' : '' }}>Dropdown (Pilihan)</option>
                    </select>
            ...
            <div class="mb-3" id="optionsField" style="display: {{ old('type') === 'dropdown' ? 'block' : 'none' }};">
                <label for="options" class="form-label">Pilihan Dropdown</label>
                <textarea class="form-control @error('options') is-invalid @enderror" id="options" name="options" rows="5" placeholder="Masukkan satu pilihan per baris">{{ old('options') }}</textarea>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="show_in_table" name="show_in_table" value="1" {{ old('show_in_table', true) ? 'checked' : '' }}>
                        <label class="form-check-label" for="show_in_table">Tampilkan di Tabel</label>
                    </div>
                </div>
                <div class="col-md-4 mb-3">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="is_searchable" name="is_searchable" value="1" {{ old('is_searchable', true) ? 'checked' : '' }}>
                        <label class="form-check-label" for="is_searchable">Bisa Dicari (Search)</label>
                    </div>
                </div>
                <div class="col-md-4 mb-3">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="is_sortable" name="is_sortable" value="1" {{ old('is_sortable', true) ? 'checked' : '' }}>
                        <label class="form-check-label" for="is_sortable">Bisa Diurutkan (Sort)</label>
                    </div>
                </div>
                <div class="col-md-4 mb-3">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="is_filterable" name="is_filterable" value="1" {{ old('is_filterable', false) ? 'checked' : '' }}>
            ...
            <div class="d-flex justify-content-between mt-3">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-save"></i> Simpan Field
                </button>
            </div>
        </form>
    </div>
